...
知识库与工具 PUT 都是各自集合的全量替换，空数组用于显式清空。V46 原始契约为知识库最多 50 个、
工具最多 20 个，下文 2026-09-06 的验收记录对应这一历史版本。

后续 [V49 数量边界统一](50_KNOWLEDGE_BINDING_LIMIT_PACKAGE_INTERFACE.md) 将知识库新写入和新任务上限统一为 20。
页面显示 `已选 N / 20`；GET 继续兼容完整读取历史 21–50 项，已选项保持可见且可明确移除。达到 20 时
不能新增选择，超过 20 时不能保存；保存前的本地拒绝不发 PUT，也不进入未知写入状态。
直接提交原始 21 项数组时，后端返回 400/`COMMON_PARAM_INVALID` 且原绑定不变；历史超限配置不会被自动截断。
工具上限保持 20。V49 验证状态与证据单列于其文档，不以本片原始 19/19 浏览器结果证明该新增边界。
...
后续 V49 对新任务检查全部持久知识库绑定总数，包括不活跃或已软删除知识库的残留绑定；超过 20 返回
409/`AGENT_BINDING_INVALID`，不新增 task/event、不调度。用户需明确删减并保存后再创建新任务；
已有 task 的 GET/Trace 与同 key、同 Agent、同原始输入的合法幂等复用仍返回原任务，不受当前绑定数量变化影响。
...
保留原草稿与已读结果。绑定顺序是 priority，读回比较同时核对成员和顺序。后续 V49 保留历史 21–50 项
的读取和移除入口，但新写入最多 20；超限时页面阻止保存，不发 PUT、不制造未知写入，也不静默删掉超出项。
...
本片也没有改变执行引擎和任务生命周期。后续 V49 还在新建前拒绝持久绑定总数超过 20 的历史配置，
包括失活或软删残留，返回409/`AGENT_BINDING_INVALID`且不创建任务；已有任务读取及合法同 key 复用不重新检查当前绑定。
...
后续 V49 的20项写入限制与历史超限修复另行验收，不能计入本片原始19/19结果。

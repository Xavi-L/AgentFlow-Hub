...
import { taskStatusLabel, recoveryUsageMessage, timestamp, pretty, message, taskFailureMessage } from '../presentation'
...
<template>
  ...
  <template v-if="trace"><section class="panel trace-overview"><div><span class="label">任务状态</span><strong class="status" :data-status="trace.task.status">{{ taskStatusLabel(trace.task) }}</strong></div><div><span class="label">步骤</span><strong>{{ trace.steps.length }}</strong></div><div><span class="label">持久事件</span><strong>{{ trace.events.length }}</strong></div><div><span class="label">{{ trace.task.recovery ? '已记录 Token 用量' : 'Token 用量 / 质量' }}</span><strong>{{ trace.task.recovery ? recoveryUsageMessage(trace.task) : `${trace.task.totalTokens} / ${trace.task.tokenUsageQuality}` }}</strong></div></section>
    <TaskRecoveryNotice :task="trace.task" />

...
## V0.2-A 前端投影：中断事实与未知用量

2026-09-12 对齐 Engine/Data/API 的 recovery 对象；本段仅扩大既有 Task/Trace 的解释展示，不新增任务重试工作流。Task 与 Trace.task 同一 recovery 结构，历史字段缺失时保持既有展示。

至少区分执行中断、调度中断、取消且执行曾中断（以及防御性取消且调度曾中断）。RUNNING 恢复任务零用量显示“当前无已记录用量，完整性未确认”，非零显示“已记录 N tokens，可能不完整”；不能把 task UNKNOWN/日志缺失变成 EXACT 或零费用。正常未开始 QUEUED 明示本地未执行与 NOT_STARTED，不推断供应商费用。

恢复任务 completedAt 是收尾时间，completedAt-startedAt 不能标为可信执行耗时或纳入普通性能比较；展示 recoveredAt 时标注收尾时间。成功日志仍可在 Trace 查阅，不能提升为权威最终答案。owner、大整数、sequence、Trace 重建、SSE 重连和 GET 收敛保持原契约，刷新/重连/重复幂等请求不会新建 task 或补发调用。

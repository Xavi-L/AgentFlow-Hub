...
2026-09-12：V0.2-A 已交付于 `9012e26`，历史实现及 A01–A17 受控 17/17 证据见切片第 9 节。V0.2-B 从该提交核对并先同步规范投影，已实现实际工作许可/迟到写入保护和有限终态保存，B01–B10 受控验收通过，具体命令、回归和失败记录见第 10 节。V47 真实成功回归仍因凭据/环境缺失 BLOCKED，不宣称全部发布证据齐备；V0.3-A 本轮从 V0.2-B 交付 `b875bde` 核对 HEAD 后实现，已先同步规范投影；A01–A13 受控验收通过，CLI 19/19、真实 PG 九类 91/91、前端及 V0.2/V48 回归通过，完整结果见 V0.3 切片第 10 节。V47 真实成功回归仍 BLOCKED；V0.3-B 未施工。
...
- V1–V16 migration 保持不可变；截至 V0.2-B，已应用的 V1–V22 均不回改；
- 新表、约束、外键和字段使用 V17+；

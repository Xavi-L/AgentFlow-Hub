...
## V0.2-B：实际工作边界与有限终态持久化

2026-09-12 施工从 A 交付 `9012e26` 开始；本节先冻结实现约束，实际 B01–B10 结果单列于切片说明。保持既有状态、快照、owner、幂等及短事务边界，V1–V22 migration 不回改。

LLM、任务前置检索的 embedding/vector、工具 handler 的实际本地工作共用独立有界许可，默认上限 4，测试可设 1。唯一拥有者按调用图确定，嵌套同一工作不得重复取得许可而自锁。等待许可受取消及该次调用 deadline 约束；取得后尚未进入工作体即取消只能归还一次。进入后仅由真实工作体 finally 归还，Future 取消/完成及外层超时均不表示工作已退出。线程池、HTTP/JDBC 有限超时与许可分别约束不同资源。

调用前检查取消和时限；正常返回先保留可用 usage 再仲裁。放弃等待后不继续决策、工具或答案发布，不将已终结日志/step 改回成功，不补造供应商用量。迟到响应不进入供应商对账流程；不合作依赖可以一直占用许可，需要安全诊断和人工受控重启。单次模型超时仍为 AGENT_LLM_TIMEOUT，整体 deadline 仍为 TIMED_OUT。

Runner 只执行一次业务。保存最初 outcome、usage、预算计数和完成观察时间，并按该时间进行 deadline 仲裁；随后的数据库重试耗时不能改写历史结果。每次终态保存先回读 task，已有终态直接接受；非终态在短事务内按持久取消意图和条件更新仲裁，不能覆盖取消。状态、答案及事件沿用同一物理事务，COMMIT 应答未知先回读，不转成另一终态。
